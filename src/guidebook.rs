...
use self::model::GuidebookSession;
...
async fn fetch_page_of_guidebook_sessions(
  config: &Config,
  url: &str,
) -> miette::Result<model::GuidebookPagedResult<GuidebookSession>> {
  ...
  let payload = resp
    .json::<model::GuidebookPagedResult<GuidebookSession>>()
    .await
    .into_diagnostic()
...
pub async fn fetch_all_guidebook_sessions(
  config: &Config,
) -> miette::Result<Vec<GuidebookSession>> {
